...
RegionView::RegionView(ArdourCanvas::Container *parent, TimeAxisView &tv,
                       const std::shared_ptr<ARDOUR::Region> &r, double spu,
                       uint32_t basic_color, bool automation)
    : TimeAxisViewItem(
          r->name(), *parent, tv, spu, basic_color, r->position(), r->length(),
          false, automation,
          (automation ? TimeAxisViewItem::ShowFrame
                      : TimeAxisViewItem::Visibility(
                            (UIConfiguration::instance().get_show_region_name()
                                 ? TimeAxisViewItem::ShowNameText
                                 : 0) |
                            TimeAxisViewItem::ShowNameHighlight |
                            TimeAxisViewItem::ShowFrame))),
      _region(r), sync_mark(nullptr), sync_line(nullptr), _editor(nullptr),
      current_visible_sync_position(0.0), valid(false), _disable_display(0),
      _pixel_width(1.0), in_destructor(false), wait_for_data(false),
      _silence_text(nullptr), _xrun_markers_visible(false),
      _cue_markers_visible(false) {
  UIConfiguration::instance().ParameterChanged.connect(
      sigc::mem_fun(*this, &RegionView::parameter_changed));

  for (const auto &s : _region->sources()) {
    s->CueMarkersChanged.connect(
        *this, invalidator(*this), [this] { update_cue_markers(); },
        gui_context());
  }
}

RegionView::RegionView(const RegionView &other)
    : sigc::trackable(other), TimeAxisViewItem(other), sync_mark(nullptr),
      sync_line(nullptr), _silence_text(nullptr), _xrun_markers_visible(false),
      _cue_markers_visible(false) {
  UIConfiguration::instance().ParameterChanged.connect(
      sigc::mem_fun(*this, &RegionView::parameter_changed));
...
RegionView::RegionView(const RegionView &other,
                       const std::shared_ptr<Region> &other_region)
    : sigc::trackable(other), TimeAxisViewItem(other), sync_mark(nullptr),
      sync_line(nullptr), _silence_text(nullptr), _xrun_markers_visible(false),
      _cue_markers_visible(false) {
  UIConfiguration::instance().ParameterChanged.connect(
...
RegionView::RegionView(ArdourCanvas::Container *parent, TimeAxisView &tv,
                       const std::shared_ptr<ARDOUR::Region> &r, double spu,
                       uint32_t basic_color, bool recording,
                       TimeAxisViewItem::Visibility visibility)
    : TimeAxisViewItem(r->name(), *parent, tv, spu, basic_color, r->position(),
                       r->length(), recording, false, visibility),
      _region(r), sync_mark(nullptr), sync_line(nullptr), _editor(nullptr),
      current_visible_sync_position(0.0), valid(false), _disable_display(0),
      _pixel_width(1.0), in_destructor(false), wait_for_data(false),
...
void RegionView::init(bool wfd) {
  ...
  if (name_highlight) {
    ...
    name_highlight->Event.connect(sigc::bind(
        sigc::mem_fun(PublicEditor::instance(),
                      &PublicEditor::canvas_region_view_name_highlight_event),
        name_highlight, this));
  }
  ...
  if (frame_handle_start) {
    ...
    frame_handle_start->Event.connect(
        sigc::bind(sigc::mem_fun(PublicEditor::instance(),
                                 &PublicEditor::canvas_frame_handle_event),
                   frame_handle_start, this));
  ...
  if (frame_handle_end) {
    ...
    frame_handle_end->Event.connect(
        sigc::bind(sigc::mem_fun(PublicEditor::instance(),
                                 &PublicEditor::canvas_frame_handle_event),
                   frame_handle_end, this));
  ...
  if (name_text) {
    ...
    name_text->Event.connect(
        sigc::bind(sigc::mem_fun(PublicEditor::instance(),
                                 &PublicEditor::canvas_region_view_name_event),
                   name_text, this));
  }
  ...
  for (long x : xrp) {
    ...
    canvas_item->set_color(
        UIConfiguration::instance().color("neutral:background"));
  ...
  /* derived class calls set_height () including RegionView::set_height() in
   * ::init() */
  // set_height (trackview.current_height());

  _region->PropertyChanged.connect(
      *this, invalidator(*this),
      std::bind(&RegionView::region_changed, this, _1), gui_context());

  /* derived class calls set_colors () including RegionView::set_colors() in
   * ::init() */
  // set_colors ();
  UIConfiguration::instance().ColorsChanged.connect(
      sigc::mem_fun(*this, &RegionView::color_handler));
...
void RegionView::set_silent_frames(const AudioIntervalResult &silences,
                                   double /*threshold*/) {
  ...
  uint32_t const color =
      UIConfiguration::instance().color_mod("silence", "silence");
  ...
  for (const auto &silence : silences) {
    ...
    cr->set_x0(trackview.editor().sample_to_pixel(silence.first -
                                                  _region->start_sample()));
    cr->set_x1(trackview.editor().sample_to_pixel(silence.second -
                                                  _region->start_sample()));
    cr->set_y0(1);
    cr->set_y1(_height - 2);
  ...
  samplecnt_t const dur =
      _region->start_sample() + _region->length_samples() - 1 - s;
  if (dur > 0) {
    shortest_audible = min(shortest_audible, dur);
  }
  ...
  _silence_text->set_x_position(
      trackview.editor().sample_to_pixel(silences.front().first -
                                         _region->start_sample()) +
      10.0);
  ...
  string text = string_compose(ngettext("%1 silent segment",
                                        "%1 silent segments", silences.size()),
                               silences.size()) +
...
gint RegionView::_lock_toggle(ArdourCanvas::Item *, GdkEvent *ev, void *arg) {
  switch (ev->type) {
...
void RegionView::lock_toggle() { _region->set_locked(!_region->locked()); }

void RegionView::region_changed(const PropertyChange &what_changed) {
...
void RegionView::region_resized(const PropertyChange &what_changed) {
  double unit_length;
...
void RegionView::reset_width_dependent_items(double pixel_width) {
  ...
  if (_xrun_markers_visible) {
    ...
    for (auto &_xrun_marker : _xrun_markers) {
      float x_pos =
          trackview.editor().sample_to_pixel(_xrun_marker.first - start);
...
void RegionView::update_xrun_markers() {
  const bool show_xruns_markers =
      UIConfiguration::instance().get_show_region_xrun_markers();
  ...
  for (auto &_xrun_marker : _xrun_markers) {
    float x_pos =
        trackview.editor().sample_to_pixel(_xrun_marker.first - start);
    _xrun_marker.second->set_x(x_pos);
    if (show_xruns_markers &&
        (_xrun_marker.first >= start && _xrun_marker.first < start + length)) {
...
RegionView::ViewCueMarker::~ViewCueMarker() { delete view_marker; }

void RegionView::update_cue_markers() {
  const bool show_cue_markers =
      UIConfiguration::instance().get_show_region_cue_markers();
  ...
  for (const auto &model_marker : model_markers) {
    ...
    if (existing == _cue_markers.end()) {
      ...
      auto *mark = new ArdourMarker(
          trackview.editor(), *group, "region mark", model_marker.text(),
          ArdourMarker::RegionCue,
          timepos_t(start.distance(model_marker.position())), true, this);
      ...
      */
      ...
      (*existing)->view_marker->set_position(
          timepos_t(start.distance(model_marker.position())));
...
void RegionView::region_opacity() { set_frame_color(); }

void RegionView::raise_to_top() { _region->raise_to_top(); }

void RegionView::lower_to_bottom() { _region->lower_to_bottom(); }
...
bool RegionView::set_position(timepos_t const &pos, void * /*src*/,
                              double *ignored) {
  double delta;
  bool ret;
...
bool RegionView::set_duration(timecnt_t const &dur, void *src) {
...
void RegionView::parameter_changed(std::string const &p) {
...
std::string RegionView::make_name() const {
  ...
  if (_region->position_time_domain() == Temporal::BeatTime) {
    str += "\xe2\x99\xab "; // BEAMED EIGHTH NOTES
  }

  if (_region->locked()) {
    str += "\xe2\x98\xa9 "; // WE'RE ONLY HUMANS!
    str += _region->name();
  } else if (_region->position_locked()) {
    str +=
        "\xe2\x86\xb9 "; // LEFTWARDS ARROW TO BAR OVER RIGHTWARDS ARROW TO BAR
    str += _region->name();
  ...
  if (_region->muted()) {
    str = std::string("\xf0\x9f\x94\x87 ") +
          str; // SPEAKER WITH CANCELLATION STROKE
  }
...
void RegionView::region_sync_changed() {
  ...
  if (!sync_mark) {
    ...
    CANVAS_DEBUG_NAME(sync_mark,
                      string_compose("sync mark for %1", get_item_name()));
    sync_line = new ArdourCanvas::Line(group);
    CANVAS_DEBUG_NAME(sync_line,
                      string_compose("sync mark for %1", get_item_name()));

    set_sync_mark_color();
  }

  /* this has to handle both a genuine change of position, a change of
     samples_per_pixel and a change in the bounds of the _region->
   */
  ...
  if (sync_offset == 0) {
    ...
    if ((sync_dir < 0) ||
        ((sync_dir > 0) && (sync_offset > _region->length()))) {
      ...
      points.push_back(
          ArdourCanvas::Duple(offset - ((sync_mark_width - 1) / 2), 1));
      points.push_back(
          ArdourCanvas::Duple(offset + ((sync_mark_width - 1) / 2), 1));
      ...
      sync_line->set(ArdourCanvas::Duple(offset, 0),
...
void RegionView::move(double x_delta, double y_delta) {
 ...
 *  which uses them. */
void RegionView::update_coverage_frame(LayerDisplay d) {
   ...
   */
  uint32_t const non_playing_color = UIConfiguration::instance().color_mod(
      "covered region", "covered region base");
...
bool RegionView::trim_front(timepos_t const &new_bound, bool no_overlap) {
  if (_region->locked()) {
    return false;
  }
  ...
  if (no_overlap) {
    ...
    std::shared_ptr<Region> region_left =
        playlist->find_next_region(pos, End, 0);
    ...
    /* Only trim region on the left if the first sample has gone beyond the left
     * region's last sample. */
    if (region_left &&
        (region_left->nt_last() > _region->position() || regions_touching)) {
  ...
  return (pos !=
          _region->position()); // return true if we actually changed something
}

bool RegionView::trim_end(timepos_t const &new_bound, bool no_overlap) {
  if (_region->locked()) {
    return false;
  }
  ...
  if (no_overlap) {
    ...
    std::shared_ptr<Playlist> playlist(_region->playlist());
    std::shared_ptr<Region> region_right =
        playlist->find_next_region(last, Start, 1);
    ...
    /* Only trim region on the right if the last sample has gone beyond the
     * right region's first sample. */
    if (region_right &&
        (region_right->position() < _region->nt_last() || regions_touching)) {
  ...
  return (last !=
          _region->nt_last()); // return true if we actually changed something
}
...
void RegionView::move_contents(timecnt_t const &distance) {
...
void RegionView::set_selected(bool yn) {
...
CueMarker RegionView::find_model_cue_marker(ArdourMarker *m) {
  for (auto &_cue_marker : _cue_markers) {
...
void RegionView::drop_cue_marker(ArdourMarker *m) {
  for (auto v = _cue_markers.begin(); v != _cue_markers.end(); ++v) {
    if ((*v)->view_marker == m) {
      delete m;
...
void RegionView::disable_display() { _disable_display++; }
